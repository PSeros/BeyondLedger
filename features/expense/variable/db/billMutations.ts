...
// Sums the parsed item line totals into a Bill totalAmount (rounded to cents). Every bill must
// carry at least one item: an item-less bill's amount is invisible to every item-level analysis
// (category breakdown, top-k, budget matching), so it would silently drop out of "where does my
// money go" while still counting in the totals. The forms guarantee a row; this is the backstop.
...
// totalAmount is always the sum of the line totals. Revalidates the list so the table, chart,
// and top-k pick up the new row.
export async function createBill(formData: FormData): Promise<void> {
  ...
  const totalAmount = sumItemTotals(items);
...
// existing ids no longer present are deleted. The Bill's totalAmount is the sum of the line totals,
// and the last item can't be removed (see sumItemTotals). Revalidates the list + this bill's detail
// so table, chart, top-k, and detail view pick up the change.
